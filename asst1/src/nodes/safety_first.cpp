...
void laser_scan(const sensor_msgs::LaserScan & laserscan)
{
	...
	for (int i=frontScanNumber; i<rightScanNumber; i++){
			if (laserscan.ranges[i] < MIN_DISTANCE){
				whichRange = i;
				isSafetyNeeded = true;
 ...
 */
    ros::Subscriber laserScan = n.subscribe("/base_scan", 1000, &laser_scan);
    ros::Subscriber origCmdVel = n.subscribe("unsafe_cmd_vel", 1000, &original_cmdVel_callBack);
